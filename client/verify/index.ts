...
// Nitro zeros PCR0/1/2 in debug mode; refuse to load an all-zero pin so we don't silently accept a debug enclave.
if (PINNED_PCR0.length === 0 || PINNED_PCR0.every((b) => b === 0)) {
  throw new Error("pinned-pcrs.json: PCR0 empty or all-zero, refusing to load");
}
